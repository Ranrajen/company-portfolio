    ...
    <>
      <section className="section-padding" style={{ paddingTop: '140px' }}>
        <div className="container-custom text-center">
          ...
          <p className="max-w-3xl mx-auto translate-x-45 " style={{ marginTop: '24px' }}>
      ...
      <section className="section-padding" style={{ paddingTop: 0 }}>
        <div className="container-custom">
          <SectionHeader title={<>Core <span className="gradient-text">Values</span></>} />
            ...
            {companyInfo.coreValues.map((value, index) => (
              <GlassCard key={value.title} className="text-center">
                <div className="w-14 h-14 flex-center rounded-full bg-brand-crimson/15 mx-auto mb-5">
                  <span className="text-xl font-bold text-brand-crimson">{index + 1}</span>
                </div>
                <h3 className="text-lg font-semibold mb-3 text-brand-crimson">{value.title}</h3>
      ...
      <section className="section-padding" style={{ paddingTop: 0 }}>
        <div className="container-custom">
          <SectionHeader title={<>Leadership <span className="gradient-text">Team</span></>} />
          <div className="grid-3-col">
            {teamData.map((member) => (
              <GlassCard key={member.id} className="text-center">
                ...
                <p className="text-brand-crimson text-sm mb-2">{member.designation}</p>
        ...
        <div className="container-custom">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            {Object.entries(companyInfo.stats).map(([key, value]) => (
              <div key={key}>
                <div className="text-3xl md:text-4xl font-bold text-brand-crimson mb-2">{value}</div>
